...
A falling block is a block similar to a generic block, but affected by gravity.
...
`collidable` | [Boolean]() | *`true`* | Determines if the block has a collison box.
...
This example creates a falling block that sounds like glass when broken, and stone when being stepped on. It also has the same hardness and blast resistance of a stone block.